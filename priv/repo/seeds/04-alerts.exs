...
# Define alert data with various types, statuses, and risk levels
alerts = [
  %{
  ...
  %{
    ...
    additional_data: %{
      ...
      "email_body_snippet" => "Please review the attached invoice and process payment within 24 hours to avoid penalties.",
      "phishing_score" => 9.2
    }
  },
  %{
...
Enum.each(all_alerts, fn alert ->
      ...
      body = case visibility do
        :internal ->
          [
            ...
            "Initial triage indicates this is a #{alert.risk_level} priority issue.",
  ...
  status_comment = case alert.status do
    :new -> 
      {
...
Enum.each(verified_alerts, fn alert ->
  ...
  if comment_count > 0 do
    ...
    IO.puts("  Alert #{alert.alert_id}: #{comment_count} comments (#{internal_count} internal, #{public_count} public, #{personal_count} personal)")
  else
