defmodule CaseManagerWeb.Plugs.ConditionalApiKey do
  ...
  def init(opts) do
    ...
    api_key_opts = AshAuthentication.Strategy.ApiKey.Plug.init([resource: User, required?: true] ++ opts)
  ...
  def call(conn, %{api_key_opts: api_key_opts}) do
    if skip_api_key_auth?(conn) do
      ...
      AshAuthentication.Strategy.ApiKey.Plug.call(conn, api_key_opts)
    end
  end
  ...
  defp skip_api_key_auth?(conn) do
    case conn.request_path do
      "/api/json/open_api" -> true
      "/api/json/swaggerui" -> true
